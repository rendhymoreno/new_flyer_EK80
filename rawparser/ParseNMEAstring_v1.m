% 2025-12-18 Adapted from Echolab --RMS

function [lat, lon, varargout] = ParseNMEAstring_v1(nmeaString, nmea_head, varargin)
...
%    varargin{1} = [false false false false];
%end
...
try
    ...
    type = upper(nmeaString(2:idx(1) - 1)); %Get header
    %idx = find(nmeaString == ',',1,'first');
	%type = upper(nmeaString(2:cPos-1)); %Get header
    
    nmeadata = nmeaString(idx(1) + 1:end); %Everything after header and comma
    ...
    switch type(3:end)
        ...
        % GLL is not an option for now    
        %{
        case 'GLL'
            if (strcmpi(type, nmea_head))
        ...
        % 2025 - 12 - 18 Vessel speed is not an option for now
        %{
        case 'VTG'
            if (rData.vSpeed)
