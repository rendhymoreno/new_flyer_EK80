...
disp('MRU and NME0 is deactivated')
disp('Turned off warnings for unsupported datagram')
pingno = 0;
envCount = 1;
paramCount = 1;
inparamCount = 1;
...
% Allocate structs 2025-12-18
blocksize = 2500;
nmeaData(blocksize) = struct('timestamp',[],'lat',[],'lon',[]);   % allocate
    ...
    while(1)
        ...
        switch(header.type)
            ...
            case 'RAW3'
                ...
                % Determine if complexsample or regular power
                s.minRange=0;
                s.minCount = 1;
                if isempty(s.complexsamples)
                    ss_sz = size(s.power, 1);
                    s.maxRange=ss_sz*s.dR;
                    
                else
                    ss_sz = size(s.complexsamples, 1);
                    s.maxRange=ss_sz*s.dR;           
                end
                
                s.maxCount = round(s.maxRange./s.dR);
                s.maxCount= min(s.maxCount,ss_sz);
...
%                s.lowdatetime = header.lowdatetime;
            ...
            %     raw = char(fread(fid,dglength-headerlength,'char')');
            %     %raw = fread(fid, dglength-headerlength, '*char')'; 
            %     %nmea.timestamp = header.datetime;
            % 
            %     % Parse NME0 String if it is ending in GGA (GPGGA/INGGA)
            %     [lat, lon, hasCheck] = ParseNMEAstring_v1(raw, {'INGGA','GPGGA'}, []);
            % 
            %     % 2025-12-18: No checksum evaluation has been done on the NMEA Data!!
            %     % If data exists
            %     if ~isempty(lat) & ~isempty(lon)
            % 
            %         % Check pre-allocated length
            %         if (nmeaCount > blocksize)
            %             newSize = blocksize*2;
            %             nmeaData(blocksize+1:newSize) = struct('timestamp',[],'lat',[],'lon',[]);
            %             blocksize = newSize;
            %         end
            % 
            %         nmeaData(nmeaCount).timestamp = header.datetime; % already a datetime
            %         nmeaData(nmeaCount).lat       = lat;
            %         nmeaData(nmeaCount).lon       = lon;
...
% 2025-12-18: Remove all empty spaces in struct!
%nmeaData(isempty(nmeaData)) = [];
emp_idx = arrayfun(@(s) isempty(s.lat), nmeaData);
nmeaData = nmeaData(~emp_idx);

data = struct('echodata', sampleData, 'filters', filterData, ...
    'config', configData, 'environ', envData, ...
    'nmea', nmeaData, 'mru', mruData ,'param', paramData, 'channelMap', channels, ...
