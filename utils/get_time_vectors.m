...
%RMS 8/19/2023
function data_vectors = get_time_vectors(data, dimen, minp, maxp, scale_range, scale_depth, tri_min_depth, tri_max_depth)
...
pings = 1:length(data.vars);
flyer_time = [data.vars.timestamp];
if flyer_time(1) > 1e15 %format is epoch millisecs (flyer timestamp), convert to serial time
    disp('[EK80 vectorization] timestamp of EK80 is in epoch/UNIX milliseconds\n')
    flyer_time = datenum(datetime(flyer_time,"ConvertFrom","epochtime",'TicksPerSecond',1e6));
end
...
for i=1:length(data.vars) %columns of vars/number of pings
    ...
    try
        ...
        X(ind) = flyer_time(i); %what is alongtrack in vars?
