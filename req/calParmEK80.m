...
cal.channel = transdata_ch.ChannelID;
cal.freq_nom = str2double(ChannelData.Frequency);
cal.freq_min = str2double(ChannelData.FrequencyMinimum);
cal.freq_max = str2double(ChannelData.FrequencyMaximum);
cal.sample_interval = ParameterData(1).SampleInterval;
...
if any(ismember(PDList',[ParameterData.PulseDuration]))
    BB_idx2 = arrayfun(@(x) find(ismember(PDList',x)==1),[ParameterData.PulseDuration]);
else
    % Look for the closest pulse duration!
    BB_idx2 = dsearchn(PDList',([ParameterData.PulseDuration]).');
end
...
cal.gain_nom = gain_list(BB_idx2); %peak gain at nominal freq
...
cal.SaCorr_log = SaCorr_list(BB_idx2); %Sa lookup table
...
cal.angleS_minor_fc_lin = 10^(cal.angleS_minor_fc_log/10); %Linear alongship angle sensitivity
cal.angleS_major_fc_lin = 10^(cal.angleS_major_fc_log/10); %Linear athwartship angle sensitivity
...
if numel(unique(BB_idx2))>1
    fprintf('[EK80 Cal] Pulse length has been modified through this file so cal parameters are not singular!\n')
else
    cal.gain_nom = unique(cal.gain_nom);
    cal.SaCorr_log = unique(cal.SaCorr_log);
    cal.gain_fc_log = unique(cal.gain_fc_log);
end
