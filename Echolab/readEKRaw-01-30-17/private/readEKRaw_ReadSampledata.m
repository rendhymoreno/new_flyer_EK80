function sampledata = readEKRaw_ReadSampledata(fid, sampledata, readTrawl)
...
%  read trawl data (older format files) or heading and transmit mode
if (readTrawl > 0)
    ...
    sampledata.heading = fread(fid,1,'float32', 'l');
    sampledata.transmitmode = fread(fid,1,'int16', 'l');
    spare = fread(fid,6,'uchar', 'l');
end
