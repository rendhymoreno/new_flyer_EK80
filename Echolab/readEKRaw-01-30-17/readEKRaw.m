...
%
%             Pitch:    Set this parameter to True to return pitch data.
%                           Default: False
%
...
%
%             Trawl:    Set this parameter to True to return trawl data. Setting
%                       this to true will read the trawl data *instead of*
%                       heading and transmit mode. Trawl data consists of trawl
%                       upper depth and trawl opening data.
%                           Default: False
%
%       The code provided by Lars Andersen (Simrad) read 12 bytes of the
%       sample data as "trawl" data and this library was written to do the
%       same. In December 2014 it was brought to my attention that this did
%       not follow the published Simrad raw file spec and this package was
%       updated to follow the published spec. This added 2 fields, heading
%       and transmitmode to the output data structure. These fields are
%       read by default but if you set the trawl keyword to true, they will
%       not be read and the reader will interpret the 12 bytes as trawl
%       data which may or may not be correct, depending on how your data
%       file was written.
%
...
%                              ---------------------------------------------------
%                                        If 'Trawl' keyword is false
%                                            heading: [1xN single]
%                                       transmitmode: [1xN int16]
%                                    {0-Active, 1-Passive, 2-Test, -1-Unknown}
%
%                                        If 'Trawl' keyword is true
%                                  trawlopeningvalid: [1xN int16]
%                               trawlupperdepthvalid: [1xN int16]
%                                    trawlupperdepth: [1xN single]
%                                       trawlopening: [1xN single]
%                              ---------------------------------------------------
...
maxSampleRange = uint16(100000); %  Effectively no maximum limit
...
maxBadBytes = 20000;            %  Keep reading 20000 bytes past a bad datagram for a good one
...
if (~rData.gpsOnly)
    ...
    sampledata = readEKRaw_AllocateSampledata(max(nSamples), rData.trawl);
    ...
    switch (dgType)
        ...
        case 'NME0'
            ...
            if rData.gps || rData.vlog || rData.vSpeed || rData.uNMEA
                ...
                switch true
                    ...
                    %  GPS Fix data / If the type is GPGGA then process
                    case rData.gps && any(strcmp(nmea.type, rData.gpsSource))
                ...
                if (pingCounter(idx) >= pingRange(1)) && (dgTime >= timeRange(1)) &&...
                    ...
                    if (nPings(idx) > arraySize(idx,1))
                        ...
                        if (rData.trawl)
                            ...
                            data.pings(idx).heading(1, nPings(idx):arraySize(idx,1)) = 0;
                            data.pings(idx).transmitmode(1, nPings(idx):arraySize(idx,1)) = 0;
                        end
                    ...
                    if (rData.trawl)
                        %  store trawl data (older raw file format)
                        ...
                        %  store heading and transmit mode
                        data.pings(idx).heading(nPings(idx)) = ...
                            sampledata.heading;
                        data.pings(idx).transmitmode(nPings(idx)) = ...
                            sampledata.transmitmode;
                    end
        ...
        otherwise
            if (maxBadBytes > 0)
                ...
                warning('readEKRaw:Datagram', ['Invalid datagram at offset ' num2str(ftell(fid)-4) ...
                ...
                while (~ok)
                    try
                        ...
                        warning('readEKRaw:Datagram', ['Unable to read next valid datagram. ' ...
                            'The raw file is truncated.']);
                        break;
                    end
                    nBytes = nBytes + 4;
                    if (strcmp(dgType, 'CON1') || strcmp(dgType, 'RAW0') || ...
                            ...
                            strcmp(dgType, 'SVP0'))
                        disp(['Found datagram: ' dgType ' at offset ' num2str(ftell(fid)-4) '(d)']);
                        ...
                        if (nBytes < (lastLen - 8)) && (strcmp(lastType, 'RAW0'))
                            ...
                            disp(['Last ping (' num2str(nPings(lastIDX)) ...
                                ') was malformed and has been dropped.']);
                        ...
                        if (nBytes > maxBadBytes); 
                            warning('readEKRaw:Datagram', ['Unable to locate a valid datagram ' ...
                                'within the provided search window. Giving up.']);
                            break;
                        end
                    end
                end
...
for idx=1:transceivercount
    if (nPings(idx) <= arraySize(idx,1))
        ...
        if (rData.trawl)
            ...
            data.pings(idx).heading(nPings(idx):end) = [];
            data.pings(idx).transmitmode(nPings(idx):end) = [];
        end
